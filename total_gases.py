...
if 'antarctic' in filename:
    ...
    location = 'Tropics'
elif 'nhmid' in filename:
    location = 'NH midlats'
elif 'shmid' in filename:
    location = 'SH midlats'
else:
    location = 'Global'
...
with open(filename,'r') as file_in:
    ...
    for line in file_in:
        ...
        PS = ds.PS
        hyai = ds.hyai
        hyai=hyai[0:len(hyai)]
        hybi = ds.hybi
        hybi=hybi[0:len(hybi)]
        
    
        if 'new_control' in line:
            O3_mmm=xray.concat([O3_mmm,O3_mmm],dim='time')        
            PS = xray.concat([PS,PS],dim='time')
            #P0 = xray.concat([P0,P0,P0,P0,P0],dim='time')
        
        Plevi = hyai*P0+hybi*PS
        Plevi = Plevi.values
        
        dp = np.zeros((O3_mmm.shape[1],24))
